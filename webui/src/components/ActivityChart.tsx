...
export type ActivityChartProps =
  | { kind: 'line'; label: string; unit?: string; points: LinePoint[] }
  | { kind: 'bar'; label: string; unit?: string; points: BarPoint[] };

type Geometry =
  | { kind: 'line'; segments: string[]; singletons: { x: number; y: number }[]; maximum: number }
  | { kind: 'bar'; bars: { x: number; y: number; width: number; height: number }[]; maximum: number };
...
function lineStats(points: LinePoint[]) {
  ...
  return { latest, minimum: observed ? minimum : null, maximum: observed ? maximum : null, observed, total: points.length };
}

function barStats(points: BarPoint[]) {
  if (!points.length) return { latest: null as number | null, minimum: null as number | null, maximum: null as number | null, observed: 0, total: 0 };
  let minimum = Infinity;
  let maximum = -Infinity;
  for (const point of points) {
    minimum = Math.min(minimum, point.value);
  ...
  return { latest: points[points.length - 1]!.value, minimum, maximum, observed: points.length, total: points.length };
}
...
export function ActivityChart(props: ActivityChartProps) {
  ...
  const stats = useMemo(() => props.kind === 'line' ? lineStats(props.points) : barStats(props.points), [props.kind, serialized]);
  ...
  const description = useMemo(() => {
    if (stats.observed === 0) return `${stats.total} 个点；无有效数值。`;
    const missing = stats.total - stats.observed;
    return `${stats.total} 个点；${stats.observed} 个有效${missing ? `；${missing} 个缺失` : ''}。最新 ${formatChartValue(stats.latest!, unit)}；最小 ${formatChartValue(stats.minimum!, unit)}；最大 ${formatChartValue(stats.maximum!, unit)}。`;
  }, [stats, unit]);

  return <div className={`activity-chart${updating ? ' is-updating' : ''}`}>
    <div className="activity-chart__stats" aria-live="polite">
      <span><strong>最新</strong> {stats.latest == null ? '--' : formatChartValue(stats.latest, unit)}</span>
      <span><strong>最小</strong> {stats.minimum == null ? '--' : formatChartValue(stats.minimum, unit)}</span>
      <span><strong>最大</strong> {stats.maximum == null ? '--' : formatChartValue(stats.maximum, unit)}</span>
      {unit ? <span className="activity-chart__unit">单位 {unit}</span> : null}
    </div>
    <div className="chart-plot">
      <div className="activity-chart__y-scale" aria-hidden="true">
        <span>{formatChartValue(geometry.maximum, unit)}</span>
        <span>0{unit === 'ms' ? ' ms' : unit ? ` ${unit}` : ''}</span>
      </div>
    ...
    {showData ? <div className="activity-chart__data">
      <table>
        ...
        <tbody>{props.kind === 'line'
          ? props.points.map((point, index) => <tr key={`${point.at}-${index}`}><th scope="row">{point.at}</th><td>{point.value === null ? '缺失' : formatChartValue(point.value, unit)}</td></tr>)
          : props.points.map((point, index) => <tr key={`${point.date}-${index}`}><th scope="row">{point.date}</th><td>{formatChartValue(point.value, unit ?? 'ms')}</td></tr>)
        }</tbody>
      </table>
    </div> : null}
  </div>;
