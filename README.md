...
`GET /metrics` 输出 Prometheus text exposition（无需登录），供 VictoriaMetrics / vmagent scrape。适合 Grafana 多序列图（按 `name` / `user_id` 图例）。

| 指标 | 含义 |
|------|------|
| `palrest_server_fps` | 最新 REST `/metrics` FPS |
| `palrest_server_frame_time_milliseconds` | 帧时间 |
| `palrest_server_current_players` / `max_players` / `uptime_seconds` | 服务器容量与运行时 |
| `palrest_online_players` | Guard 轮询在线人数 |
| `palrest_player_online{user_id,name}` | 每名玩家是否在线 |
| `palrest_player_ping_milliseconds{user_id,name}` | **仅在线**玩家延迟（ms） |
| `palrest_player_level{user_id,name}` | 仅在线等级 |
| `palrest_player_used_seconds` / `remaining_seconds` / `limit_seconds` | 防沉迷用时 |

示例 scrape（vmagent / prometheus）：
...
Grafana 示例：`palrest_player_ping_milliseconds` 图例 `{{name}}` 即可一屏看到全部在线玩家延迟曲线。不写 IP、坐标；仍建议只在内网 scrape，勿把 API 直接挂公网。
