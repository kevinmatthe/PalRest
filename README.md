...
回放插值只用于画面，不产生新的事实记录。不同轨迹段、服务器重启、超过 5 分 30 秒的缺口及至少 50,000 游戏单位的疑似跳跃均不连接，可跳到下一次观测。历史回放按游标附近的 10 分钟片段加载，前后附带 5 分 30 秒的连续性数据，并预取下一段；最多缓存 3 段。时间轴覆盖完整记录范围，跳转到未缓存时段会显示缓冲并暂停推进，数据完整到达后继续。单段超过 500 条时自动拆分查询，无法完整加载则明确报错，不将截断当成观测缺口。回放中的游玩小结与热度仅统计当前已缓冲区间内、游标之前的证据。
...
切换到「游玩小结」可查看当前观察窗口的有效时长、覆盖率、未知时长、已观测路径、REST 等级变化及六项存档成长曲线。存档等级和经验保留各自观测时间，不与 REST 等级混用。数字可展开保存的快照或位置证据，并跳转到变化区间终点。图表实点表示观测，虚线只连接可比较记录，不表示变化的准确发生时间；缺失、回档与世界切换会断开比较。小结自动分段加载所选观察窗口，历史小结统计窗口起点至游标时刻，不随地图局部缓存缩短。中断前已确认的成长仍保留，跨边界只展示各有效区间的变化合计，不冒充窗口净变化。原始观测表分页展示；加载超限或失败会报错，不把截断后的记录当作完整窗口。小结不是完整游玩会话。
...
当容器把世界目录直接挂载为 `/Save` 时，路径会丢失原世界 GUID。应在 `save.world_id` 显式填写宿主机存档目录的 GUID（非零 32 位十六进制，可带标准连字符）；后端会传给 worker 的 `--world-id`。留空时仍从存档路径的父目录识别，识别失败会暂停成长比较，不能解释为没有成长。修改后需重启更新后的后端。首次带有效世界标识的快照建立新基线，后续一致快照才生成变化；历史 `world_unknown` 快照不会自动重写或补算。
