...
func TestWorldPOIProviderMergesLiveAndSave(t *testing.T) {
	...
	live.Update(palworld.GameDataSnapshot{
		...
		PalBoxes: []palworld.LivePalBox{
			// Far from save camp so both are kept (not the same physical base).
			{ID: "pb-g1-1", GuildID: "g1", GuildName: "Alpha", X: 100_000, Y: 20},
		},
	})
	provider := &WorldPOIProvider{
		Save: savePOIFake{result: store.PlayerWorldPOIs{
			...
			POIs: []store.WorldPOI{{
				...
				X: 11, Y: 21, GuildName: "Alpha", GuildID: "g1", Source: "save_import",
			}},
		}},
		Live: live,
	}
